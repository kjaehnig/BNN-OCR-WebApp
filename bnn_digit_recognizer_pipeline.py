...
def setup_training_env(bs=512):
    # Use the EMNIST digit datasets
    dataset_type='by'
    dspath = '/mnt/g/WSL/downloaded_ml_data/emnist-all/'

    train, test = tf.keras.datasets.emnist.bymerge.load_data()
    ...
    train_datagen = tf.data.Dataset.from_tensor_slices((train_images, train_labels)).shuffle(10000).batch(bs)
    test_datagen = tf.data.Dataset.from_tensor_slices((test_images, test_labels)).shuffle(5000).batch(bs)
...
def create_bnn(n_labels=47, DO=0.25):
    # global n_labels
